...
def insert_idx(sentence_lst: list, sentence: str) -> int:
    ...
    for idx, s in enumerate(sentence_lst):
        # parsing values in format "file_name offset"
        file = s.split()[0]
        offset = int(s.split()[1])
        if file_data[file][offset] > sentence:
            return idx
...
def add_to_dict(key: str, sentence: str, file: str, offset: int):
    ...
    if key in map_data.keys():
        if len(map_data[key]) < AMOUNT_SENTENCES:
            ...
            map_data[key] = map_data[key][:index] + [file + " " + str(offset)] + map_data[key][index:]
    else:
        map_data[key] = [file + " " + str(offset)]
